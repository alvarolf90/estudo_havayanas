...
ui <- page_sidebar(
  ...
  sidebar = sidebar(
    ...
    accordion(
      open = c("Sequências", "Levadas", "Convenções", "Acompanhamento", "Mixagem"),
      
      accordion_panel("Sequências",
                      tags$p("Monte uma sequência fixa de passos (sem sorteio) e gere um link para compartilhar. Quem abrir o link toca exatamente essa sequência, sem precisar mexer em nada.", style = "font-size: 0.82em; color: #7f8c8d; margin-bottom: 10px;"),
                      selectInput("seq_tipo_passo", "Tipo de passo:", choices = c("Levada" = "levada", "Convenção" = "convencao")),
                      conditionalPanel(
                        condition = "input.seq_tipo_passo == 'levada'",
                        selectInput("seq_padrao_levada", "Levada:", choices = setdiff(levadas_disponiveis, "Variada")),
                        numericInput("seq_compassos_passo", "Nº de compassos:", value = 8, min = 2, max = 32, step = 2)
                      ),
                      conditionalPanel(
                        condition = "input.seq_tipo_passo == 'convencao'",
                        selectInput("seq_padrao_convencao", "Convenção:", choices = convencoes_disponiveis)
                      ),
                      div(class = "btn-container", actionButton("btn_add_passo", "+ Adicionar à sequência", icon = icon("plus"), class = "btn-sm btn-outline-primary")),
                      uiOutput("lista_passos_sequencia"),
                      hr(style = "margin: 14px 0;"),
                      tags$label("Nome do exercício:", style = "font-weight: bold; font-size: 0.85em;"),
                      textInput("seq_nome", NULL, placeholder = "Ex.: Aula 3 - Baião e Xote"),
                      tags$p("Velocidade, foco e banda usados serão os que estiverem configurados nas abas abaixo no momento de gerar o link.", style = "font-size: 0.78em; color: #7f8c8d; margin: 4px 0 10px 0;"),
                      div(class = "btn-container", actionButton("btn_gerar_link_seq", "Gerar link e QR code", icon = icon("link"), class = "btn-sm btn-primary")),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$btn_gerar_link_seq, {
    ...
    if (length(input$acompanhamento_ativo) > 0) {
      for (inst in input$acompanhamento_ativo) {
        id_inst <- gsub(" ", "_", inst)
        acomp_list[[inst]] <- list(
          levadas = input[[paste0("acomp_lev_", id_inst)]],
          convencoes = input[[paste0("acomp_conv_", id_inst)]],
          vol = input[[paste0("vol_", id_inst)]]
        )
      }
    }
    ...
    seq_obj <- list(
      ...
      vol_principal = input$vol_principal,
      acompanhamento_ativo = input$acompanhamento_ativo,
      acompanhamento = acomp_list,
      passos = passos
    )
  ...
  observeEvent(input$sequencia_da_url, {
    ...
    if (!is.null(seq$vol_principal)) updateSliderInput(session, "vol_principal", value = as.numeric(seq$vol_principal))
    
    acomp_ativo_seq <- seq$acompanhamento_ativo
    acomp_detalhe_seq <- seq$acompanhamento
    
    shinyjs::delay(150, {
      if (!is.null(acomp_ativo_seq)) updateCheckboxGroupInput(session, "acompanhamento_ativo", selected = acomp_ativo_seq)
      
      shinyjs::delay(350, {
        if (!is.null(acomp_detalhe_seq)) {
          for (inst in names(acomp_detalhe_seq)) {
            id_inst <- gsub(" ", "_", inst)
            sub <- acomp_detalhe_seq[[inst]]
            updateCheckboxGroupInput(session, paste0("acomp_lev_", id_inst), selected = sub$levadas)
            updateCheckboxGroupInput(session, paste0("acomp_conv_", id_inst), selected = sub$convencoes)
            if (!is.null(sub$vol)) updateSliderInput(session, paste0("vol_", id_inst), value = as.numeric(sub$vol))
          }
        }
      })
    })
